// src/app/(dashboard)/dashboard/sponsorship/[id]/edit/page.tsx
...
import type { SponsorshipCase, SponsorshipStatus } from "@/types/sponsorship";
import { getImageUrl } from "@/lib/utils/imageUrl";

export default function EditSponsorshipPage({ params }: { params: { id: string } }) {
  ...
  const loadCase = async () => {
    try {
      ...
      setCurrentImage(data.caseImage);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await sponsorshipCasesApi.update(params.id, formData);
      toast.success("تم تحديث الحالة بنجاح");
      router.push(`/dashboard/sponsorship/${params.id}`);
    } catch (error: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">تعديل حالة الكفالة</h1>
          <p className="text-gray-600 mt-1">تحديث بيانات حالة {patientName}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                صورة الحالة {!imagePreview && currentImage && "(الصورة الحالية)"}
              </label>
              {imagePreview ? (
                ...
                <div className="relative w-full h-64 rounded-lg overflow-hidden border-2 border-gray-200">
                  ...
                  <label className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center cursor-pointer hover:bg-opacity-60 transition-all">
                    <div className="text-center text-white">
                      ...
                      <span className="text-sm">اضغط لتغيير الصورة</span>
                    </div>
                    <input
                      type="file"
                      accept="image/*"
